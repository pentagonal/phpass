...
class PasswordHash
{
     ...
     */
    public function __construct($iterationCountLog = 8, $portable_hashes = false)
    {
        $this->ito64 = './0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';

        if ($iterationCountLog < 4 || $iterationCountLog > 31) {
            $iterationCountLog = 8;
        }

        $this->iteration_count_log = $iterationCountLog;
    ...
    private function encode64($input, $count)
    {
        ...
        do {
            $value = ord($input[$i++]);
            $output .= $this->ito64[$value & 0x3f];
            if ($i < $count) {
                $value |= ord($input[$i]) << 8;
            }

            $output .= $this->ito64[($value >> 6) & 0x3f];
            if ($i++ >= $count) {
                break;
            }
            if ($i < $count) {
            ...
            $output .= $this->ito64[($value >> 12) & 0x3f];
            if ($i++ >= $count) {
                break;
            }

            $output .= $this->ito64[($value >> 18) & 0x3f];
        } while ($i < $count);
    ...
    {
        $output = '$P$';
        $output .= $this->ito64[min($this->iteration_count_log +
    ...
    private function cryptPrivate($password, $setting)
    {
        ...
        $count_log2 = strpos($this->ito64, $setting[3]);
    ...
    {
        $count_log2 = min($this->iteration_count_log + 8, 24);
        # This should be odd to not reveal weak DES keys, and the
        # maximum valid value is (2**24 - 1) which is odd anyway.
        $count = (1 << $count_log2) - 1;

        $output = '_';
        $output .= $this->ito64[$count & 0x3f];
        $output .= $this->ito64[($count >> 6) & 0x3f];
        $output .= $this->ito64[($count >> 12) & 0x3f];
        $output .= $this->ito64[($count >> 18) & 0x3f];
    ...
    private function genSaltBlowFish($input)
    {
        ...
        $output = '$2a$';
        $output .= chr(ord('0') + $this->iteration_count_log / 10);
        $output .= chr(ord('0') + $this->iteration_count_log % 10);
        $output .= '$';
        $i = 0;
        do {
     ...
     * Hash the password
     *
     * @param  string $plainPassword the password the be random hashed
    ...
    public function hashPassword($plainPassword)
    {
        ...
        if (CRYPT_BLOWFISH == 1 && ! $this->portable_hashes) {
            ...
            $hash =
                crypt($plainPassword, $this->genSaltBlowFish($random));
            if (strlen($hash) == 60) {
                return $hash;
            }
        }
        ...
        if (CRYPT_EXT_DES == 1 && ! $this->portable_hashes) {
            ...
            $hash =
                crypt($plainPassword, $this->genSaltExtended($random));
            if (strlen($hash) == 20) {
                return $hash;
            }
        }
     ...
     */
    public function checkPassword($plainPassword, $storedHash)
    {
        if (!is_string($plainPassword) || !is_string($storedHash)) {
            return false;
        }
        $hash = $this->cryptPrivate($plainPassword, $storedHash);
        if ($hash[0] == '*') {
